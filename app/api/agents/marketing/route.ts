...
export async function POST(request: Request) {
  ...
  const job = await prismaInfra.agentJob.create({ data: { agentName: 'marketing', triggeredBy: 'admin', status: 'RUNNING' } });
  ...
  try {
    ...
    const agent = new MarketingAgent(ctx as any);
    ...
    return NextResponse.json({ error: 'Marketing Agent run failed' }, { status: 500 });
  }
}
