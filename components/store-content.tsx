...
export function StoreContent({ initialProducts }: StoreContentProps) {
  ...
  const [onlyInStock, setOnlyInStock] = useState(
    Boolean(searchParams.get('inStock'))
  );
  ...
  }, [query, category, sort, priceRange, onlyInStock, onlySale, router]);
  ...
  const filtered = useMemo(() => {
    ...
    return list;
  }, [query, category, sort, priceRange, onlyInStock, onlySale, products]);
  ...
  return (
    <div className="min-h-screen bg-black text-white pt-20">
      {session?.user && (
        <div className="text-sm text-cyan-400 mb-2">Welcome, {session.user.name || session.user.email}</div>
      )}
      {/* ── PAGE HEADER ─────────────────────────────────── */}
